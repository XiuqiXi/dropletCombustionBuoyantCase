...
plt.scatter(data_rawData[:,0],np.sqrt(data_rawData[:,1])*10**3, label = 'experimental data', color = 'r')
#,ls = '--'
plt.xlabel('Time(s)')
...
with open('./postProcessing/fieldMinMax1/0/fieldMinMax.dat') as txtData:
    ...
    for i in range(n):
        ...
        flamePosition_temp = np.sqrt(position[0]*position[0]+(math.fabs(position[1])-0.05)**2)
